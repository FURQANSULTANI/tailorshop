...
public static class EmailBackup
{
    ...
    public static EmailResult Send(EmailSettings settings)
    {
        ...
        try
        {
            ...
            zip = Path.Combine(tempDir, $"GoldenTailor_Backup_{stamp}.zip");
            ...
            using var message = new MailMessage
            {
                ...
                Subject = $"Golden Tailor Backup — {DateTime.Now:dd-MM-yyyy hh:mm tt}",
                Body    = $"Automatic database backup from Golden Tailor.{Environment.NewLine}{Environment.NewLine}" +
                          $"Created : {DateTime.Now:dd-MM-yyyy hh:mm tt}{Environment.NewLine}" +
                          $"Size    : {result.SizeText}{Environment.NewLine}{Environment.NewLine}" +
                          "Keep this file safe. To restore, open Backup & Restore in Golden Tailor, " +
    ...
    public static EmailResult TestConnection(EmailSettings settings)
    {
        ...
        try
        {
            ...
            {
                From    = new MailAddress(settings.Username, settings.FromName),
                Subject = "Golden Tailor — settings test (no backup attached)",
